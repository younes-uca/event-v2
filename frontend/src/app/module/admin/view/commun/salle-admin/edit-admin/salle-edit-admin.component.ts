...
@Component({
  selector: 'app-salle-edit-admin',
  templateUrl: './salle-edit-admin.component.html'
})
export class SalleEditAdminComponent extends AbstractEditController<SalleDto, SalleCriteria, SalleService>   implements OnInit {
   ...
   public async openCreateBlocOperatoir(blocOperatoir: string) {
        const isPermistted = await this.roleService.isPermitted('BlocOperatoir', 'edit');
        if(isPermistted) {
             this.blocOperatoir = new BlocOperatoirDto();
