...
//                log.info("new user connected with id =  " + userId);
    ...
    private List<EvenementRedis> searchObjectsByReference(String ref) {
        Flux<EvenementRedis> evenementRedisFlux = getEventsByReference(ref);
    ...
    public Flux<EvenementRedis> getEventsByReference(String reference) {
    ...
    public Flux<EvenementRedis> searchObjectsByReferenceInRedis(String reference) {
        ...
        return template.opsForHash()
                .get(String.valueOf(reference), reference)
                .flux()
                .flatMap(objects -> {
                    if (objects instanceof List) {
                        return Flux.fromIterable((List<EvenementRedis>) objects);
                    } else {
                        // Handle the case where only a single object is retrieved
                        return Flux.just((EvenementRedis) objects);
                    }
                });
    }
